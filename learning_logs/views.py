...
def index(request):
	"""Головна сторінка "Learning Log"."""
	return render(request, 'learning_logs/index.html')

@login_required
def topics(request):
	"""Відобразити всі теми"""
...
def new_entry(request, topic_id):
	"""Додати новий допис для конкретної теми"""
	topic = get_object_or_404(Topic, id=topic_id)
	check_topic_owner(topic, request)
	if request.method != 'POST':
...
def edit_entry(request, entry_id):
	...
	check_topic_owner(topic, request)
	if request.method != 'POST':
		# Початковий запит; попередньо заповнити форму поточним записом
		form = EntryForm(instance=entry)
	else:
		# Надіслані дані POST; обробити дані
